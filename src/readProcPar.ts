...
export class Header{
  ...
  public constructor (lineIn:string){
    ...
    this.name = line[0];
    this.subType = line[1];
    this.basicType = parseInt(line[2]);
    this.maxValue = parseInt(line[3]);
    this.minValue = parseInt(line[4]);
    this.stepSize = parseInt(line[5]);
    this.gGroup = line[6];
    this.dGroup = line[7];
...
export class Param extends Header {
  ...
  public constructor(lines:Lines){
    ...
    let numOfLines = parseInt(line2.split(' ')[0]);

    if(this.basicType === 1){ /* real num, one line */
    ...
    this.enumerable = parseInt(line3.split(' ')[0]);

    if (this.enumerable !== 0){ /* if 0 we dont do anything */
      if (this.basicType === 1){     // reals
